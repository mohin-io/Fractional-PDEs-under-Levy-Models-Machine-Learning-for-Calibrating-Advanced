...
async def load_resources():
    ...
    if not os.path.exists(SCALER_PATH):
        raise RuntimeError(
            f"Scaler not found at {SCALER_PATH}. Please ensure it's saved during training."
        )
...
@app.post(
    "/calibrate", summary="Calibrate Levy model parameters from an option price surface"
)
async def calibrate_parameters(input_data: OptionSurfaceInput):
    ...
    if model is None or scaler_X is None:
        raise HTTPException(
            status_code=503, detail="Model or scaler not loaded. API is not ready."
        )

    option_surface_array = np.array(input_data.option_prices).reshape(
        1, -1
    ...
    # Make prediction
    predicted_params = model.predict(option_surface_scaled)[
        0
