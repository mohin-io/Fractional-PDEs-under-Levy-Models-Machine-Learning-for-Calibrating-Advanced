...
def run_sensitivity_analysis(param_to_vary="sigma", num_steps=10, range_factor=0.2):
    ...
    try:
        ...
        print(
            f"Warning: Feature '{param_to_vary}' not found. Using 'price_0_0' as default."
        )
    ...
    variation_range = np.linspace(
        mean_val - range_factor * std_val, mean_val + range_factor * std_val, num_steps
    )
    ...
    original_variation_range = scaler_X.inverse_transform(
        np.array(
            [
                base_input[:param_idx].tolist()
                + [val]
                + base_input[param_idx + 1 :].tolist()
                for val in variation_range
            ]
        )
    )[:, param_idx]
    ...
    print(
        f"Varying '{param_to_vary}' from {original_variation_range.min():.4f} to {original_variation_range.max():.4f} (Original Scale)"
    )
    ...
    for i, target_param in enumerate(target_cols):
        sns.lineplot(
            x=original_variation_range,
            y=predicted_params_over_range[:, i],
            label=f"Predicted {target_param}",
            marker="o",
        )
    ...
    print(
        f"This plot illustrates how the predicted Levy model parameters change as a specific input feature ('{param_to_vary}') is varied, while other features are held constant at their average values."
    )
    print(
        "Each line represents a different Levy model parameter. The slope and direction of these lines indicate the sensitivity:"
    )
    print(
        "  - A steep slope suggests high sensitivity: a small change in the input feature leads to a large change in the predicted parameter."
    )
    print(
        "  - A flat line suggests low sensitivity: the predicted parameter is relatively robust to changes in this input feature."
    )
    print(
        "  - The direction (positive or negative slope) shows whether the predicted parameter increases or decreases with the input feature."
    )
    print(
        "This analysis is crucial for understanding the model's behavior, identifying influential input features, and assessing the stability of the calibration process. For instance, if a parameter is highly sensitive to a noisy input, it might lead to unstable calibrations in real-world scenarios."
    )
