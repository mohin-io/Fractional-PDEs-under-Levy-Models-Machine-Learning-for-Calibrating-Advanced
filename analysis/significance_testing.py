...
def run_significance_testing():
    ...
    for i, param_name in enumerate(targets_df.columns):
        ...
        print(
            f"  Paired t-test (Abs Errors vs 0): t-statistic = {t_stat:.4f}, p-value = {p_value:.4f}"
        )
        if p_value < 0.05:
            print(
                "  Conclusion: Mean absolute error is significantly different from zero (p < 0.05)."
            )
        else:
            print(
                "  Conclusion: Mean absolute error is NOT significantly different from zero (p >= 0.05)."
            )
    ...
    for i, param_name in enumerate(targets_df.columns):
        print(
            f"  {param_name}: Mean Error = {np.mean(errors[:, i]):.4f}, Std Dev Error = {np.std(errors[:, i]):.4f}"
        )
    ...
    print(
        "The histograms of errors show the frequency distribution of prediction errors for each parameter. Ideally, these should be centered around zero and resemble a normal (bell-shaped) distribution, indicating unbiased predictions."
    )
    print(
    ...
    print(
        "Both plots help in visually assessing the quality and statistical properties of the model's errors, which is crucial for understanding its reliability."
    )
