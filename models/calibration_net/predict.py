...
from models.calibration_net.model import (
    build_mlp_model,
)  # Needed to load custom model architecture
...
def predict_parameters(option_surface_data, scaler_X=None, target_cols=None):
    ...
    model = tf.keras.models.load_model(
        MODEL_PATH, custom_objects={"build_mlp_model": build_mlp_model}
    )

    # Preprocess input data using the same scaler used during training
    if scaler_X is None:
        print(
            "Warning: No scaler provided. Fitting a new scaler on dummy data. This is not recommended for production."
        )
...
if __name__ == "__main__":
    ...
    dummy_option_surface = np.random.rand(
        1, 200
    )  # 1 sample, 20 strikes * 10 maturities

    # Dummy scaler (in a real scenario, load the saved scaler)
    dummy_scaler = StandardScaler()
    dummy_scaler.fit(
        np.random.rand(100, 200)
    )  # Fit on some data with same feature dimensions
    ...
    try:
        predicted_params = predict_parameters(
            dummy_option_surface, scaler_X=dummy_scaler, target_cols=dummy_target_cols
        )
